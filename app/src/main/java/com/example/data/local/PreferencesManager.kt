...
class PreferencesManager(context: Context) {
    ...
    var equalizerPreset: String
        get() = prefs.getString("KEY_EQ_PRESET", "Default") ?: "Default"
        set(value) = prefs.edit().putString("KEY_EQ_PRESET", value).apply()

    var bassBoost: Float
        get() = prefs.getFloat("KEY_BASS_BOOST", 10f)
        set(value) = prefs.edit().putFloat("KEY_BASS_BOOST", value).apply()

    var surroundSound: Float
        get() = prefs.getFloat("KEY_SURROUND", 0f)
        set(value) = prefs.edit().putFloat("KEY_SURROUND", value).apply()

    var reverbPreset: String
        get() = prefs.getString("KEY_REVERB_PRESET", "None") ?: "None"
        set(value) = prefs.edit().putString("KEY_REVERB_PRESET", value).apply()
    ...
    var selectedTheme: String
        get() = prefs.getString("KEY_SELECTED_THEME", "AMOLED Black") ?: "AMOLED Black"
    ...
    var widgetSizePreset: String
        get() = prefs.getString("KEY_WIDGET_SIZE", "Auto (Responsive)") ?: "Auto (Responsive)"
        set(value) = prefs.edit().putString("KEY_WIDGET_SIZE", value).apply()
    ...
    fun restoreDefaultSettings() {
        prefs.edit()
            .putString("KEY_SELECTED_THEME", "AMOLED Black")
            .putString("KEY_SELECTED_LANGUAGE", "English (US)")
            .putString("KEY_ACCENT_COLOR", "#A855F7")
            .putBoolean("KEY_EQ_ENABLED", false)
            .putString("KEY_EQ_PRESET", "Default")
            .putFloat("KEY_BASS_BOOST", 10f)
            .putFloat("KEY_SURROUND", 0f)
            .putString("KEY_REVERB_PRESET", "None")
            .putBoolean("KEY_LOUDNESS", true)
            .putString("KEY_BAND_GAINS", "-5.0,-4.0,7.0,8.0,12.0")
            .apply()
    }
    ...
    fun getBandGains(): List<Float>? {
        ...
        return try {
            val list = str.split(",").map { it.toFloat() }
